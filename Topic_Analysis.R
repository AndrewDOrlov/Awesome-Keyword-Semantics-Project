# загрузка данных
secure_df <- read.csv("C:/Users/andre/Dev/R/Topic_Analysys/data/Secure_df_postprocessed.csv",
                      na = "")
...
# из датасета убираются данные за неполные кварталы и полугодия:
secure_df <- subset(secure_df, !Month == "2023-01-01" &
                       !Month == "2021-04-01" &
                       !Quarter == "2021-04-01")
...
ggplot(month_count_wide_df, aes(x = Month, y = утечки)) +
  ...
  annotate("Text", label = "Точка изменения тренда",
           x = month_count_wide_df$Month[pt$estimate] +40, y = -2, 
           size = 4, colour = "red") +
